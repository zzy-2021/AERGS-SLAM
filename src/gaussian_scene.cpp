...
void GaussianScene::addCamera(GaussianView& camera)
{
    this->cameras_.emplace(camera.camera_id_, camera);
}

GaussianView& GaussianScene::getCamera(camera_id_t cameraId)
{
    return this->cameras_[cameraId];
}

void GaussianScene::remove_camera_outside_window()
{
    std::unique_lock<std::mutex> lock_kfs(this->mutex_kfs_);
    int n_to_removed = this->keyframes_.size()-200;   // 150 ->  250
    if (n_to_removed<=0)
        return;

    // 从begin()开始删除最旧的n个元素
    auto it = this->keyframes_.begin();
    while (n_to_removed-- > 0 && it != this->keyframes_.end()) {
        this->keyframes_saved.insert(*it);

//        auto a = it->second->img_undist_;
//        cv::imshow("remove", a);
//        cv::waitKey(1);

//        std::cout<<"removed"<<std::endl;

        it = this->keyframes_.erase(it); // 删除当前元素并指向下一个
    }

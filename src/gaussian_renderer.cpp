...
GaussianRenderer::render(
    ...
    auto screenspace_points = torch::zeros_like(pc->getXYZ(),
                                                torch::TensorOptions().dtype(pc->getXYZ().dtype()).requires_grad(
                                                        true).device(torch::kCUDA));
    try {
        screenspace_points.retain_grad();
    }
    catch (const std::exception &e) { ; // pass
    }
    ...
    if (use_override_color) {
        ...
        if (pipe.convert_SHs_) {
            ...
            torch::Tensor dir_pp = (pc->getXYZ() -
                                    viewpoint_camera->camera_center_.repeat({pc->getFeatures().size(0), 1}));
    ...
    //torch::Tensor cam_rot_delta_ = torch::zeros({3}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA)).requires_grad_();
    //torch::Tensor cam_trans_delta_ = torch::zeros({3}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA)).requires_grad_();
    torch::Tensor cam_rot_delta_ = viewpoint_camera->cam_rot_delta_;
    torch::Tensor cam_trans_delta_ = viewpoint_camera->cam_trans_delta_;

    // 检查cam_rot_delta_
    assert(cam_rot_delta_.numel() == 3 && "cam_rot_delta_元素数量必须为3");
    //assert(cam_rot_delta_.sizes() == torch::Size{3} && "cam_rot_delta_形状必须为(3)");
    assert(cam_rot_delta_.requires_grad() && "cam_rot_delta_必须启用梯度");
//
//    // 检查cam_trans_delta_
    assert(cam_trans_delta_.numel() == 3 && "cam_trans_delta_元素数量必须为3");
    //assert(cam_trans_delta_.sizes() == torch::Size({3}) && "cam_trans_delta_形状必须为(3)");
    assert(cam_trans_delta_.requires_grad() && "cam_trans_delta_必须启用梯度");

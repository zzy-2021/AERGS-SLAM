...
void GaussianKeyframe::setPose( const Eigen::Matrix4d& Tcw)
{

    Eigen::Matrix4d T = Tcw.inverse();
...
void GaussianKeyframe::setCameraParams(const GaussianView& camera)
{
...
void GaussianKeyframe::updatePoseFromRender()
{
    // tensor to le algbra
    torch::Tensor delta_rot = this->cam_rot_delta_.to(torch::kCPU);
    torch::Tensor delta_trans = this->cam_trans_delta_.to(torch::kCPU);

    Eigen::Vector3f omega(delta_rot[0].item<float>(),delta_rot[1].item<float>(),delta_rot[2].item<float>() );
    Eigen::Vector3f upsilon(delta_trans[0].item<float>(),delta_trans[1].item<float>(),delta_trans[2].item<float>() );

    Eigen::Matrix<float, 6, 1> se3;
    se3.head<3>() = upsilon;  // 平移部分在前
    se3.tail<3>() = omega;    // 旋转部分在后

    // 计算se(3)的指数映射，得到SE(3)变换
    Eigen::Matrix4f delta_Twc = Sophus::SE3f::exp(se3).matrix();

    // 更新Twc及Gaussian的 view 信息
    Eigen::Matrix4f New_Twc = delta_Twc* this->Twcf_;
    ...
    if (this->set_pose_ && this->set_camera_) {
        ...
        this->cam_rot_delta_.zero_();
        this->cam_trans_delta_.zero_();
    }
    else if (!this->set_pose_ && this->set_camera_) {
        std::cerr << "Could not compute transform tensors for keyframe " << this->fid_ << " because POSE is not set!" << std::endl;
    }

...
Frame::Frame(int frame_id, bool pose_fixed, CameraPtr camera, double timestamp, cv::Mat imgleft, std::string name) :
    ...
    if (imgleft.channels() != 3)
        cv::cvtColor(imgleft, imgRGB, cv::COLOR_GRAY2RGB);
    else
        imgleft.copyTo(imgRGB);
...
int Frame::AddRightFeatures(Eigen::Matrix<float, 259, Eigen::Dynamic> &features_right,
    ...
    MatchLines(_points_on_lines, points_on_line_right, matches, _features.cols(), features_right.cols(),
               line_matches);
...
Eigen::Vector3d Frame::GetColor(int idx)   // 获得cv::Mat图像中，idx的特征对应的颜色。
{
    if (idx >= _features.cols() || idx < 0)
        return Eigen::Vector3d(0.5, 0.5, 0.5);

    const auto& kps = _keypoints[idx];

    // 将浮点数坐标转换为整数，并确保在图像范围内
    int x = static_cast<int>(std::round(kps.pt.x));
    int y = static_cast<int>(std::round(kps.pt.y));

    // 检查坐标是否在图像范围内
    if (x < 0 || x >= imgRGB.cols || y < 0 || y >= imgRGB.rows)
        return Eigen::Vector3d(0.5, 0.5, 0.5);
    // 获取RGB值（假设imgRGB是归一化的浮点型图像）
    const cv::Vec3f& rgb = imgRGB.at<cv::Vec3f>(y, x);
    // 直接返回RGB值（已归一化）

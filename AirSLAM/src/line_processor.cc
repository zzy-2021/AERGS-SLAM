...
float PointLineDistance(Eigen::Vector4f line, Eigen::Vector2f point) {
    ...
    float x2 = line(2);
    float y2 = line(3);
    float d = (std::fabs((y2 - y1) * x0 + (x1 - x2) * y0 + ((x2 * y1) - (x1 * y2)))) /
              (std::sqrt(std::pow(y2 - y1, 2) + std::pow(x1 - x2, 2)));
    return d;
}

double
CVPointLineDistance3D(const std::vector<cv::Point3f> points, const cv::Vec6f &line, std::vector<float> &dist) {
...
void MatchLines(const std::vector<std::map<int, double>> &points_on_line0,
                const std::vector<std::map<int, double>> &points_on_line1,
    ...
    for (size_t j = 0; j < line_num1; j++) {
        ...
        float score = (float) (col_max_val * col_max_val) /
                      std::min(points_on_line0[col_max_location].size(), points_on_line1[j].size());
...
void SortPointsOnLine(std::vector<Eigen::Vector2d> &points, std::vector<size_t> &order, bool sort_by_x) {
    ...
    if (sort_by_x) {
        std::sort(order.begin(), order.end(),
                  [&points](size_t i1, size_t i2) { return points[i1](0) < points[i2](0); });
    } else {
        std::sort(order.begin(), order.end(),
                  [&points](size_t i1, size_t i2) { return points[i1](1) < points[i2](1); });
    }
}
...
bool TriangulateByTwoFrames(const Eigen::Vector4d &line_2d1, const Eigen::Matrix4d &pose1,
                            const Eigen::Vector4d &line_2d2, const Eigen::Matrix4d &pose2, const CameraPtr &camera,
                            Line3DPtr line_3d) {

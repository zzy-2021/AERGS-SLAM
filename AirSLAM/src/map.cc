...
void Map::InsertKeyframe(FramePtr frame) {
    ...
    if (_keyframes.size() < 2) {
        ...
        // 把窗口内的点，keyframe 添加进入opr
        MappingOperation opr(MappingOperation::OprType::LocalMappingBA);
        LocalMapOptimization(frame, opr);

        this->pushMappingOperation(opr);

//        std::cout << "keyframe size:" << opr.associatedKeyFrames().size() << std::endl;
//        std::cout << "mappoint size:" << std::get<0>(opr.associatedMapPoints()).size() << std::endl;
...
void Map::DeleteKeyframe(FramePtr frame) {
    ...
    std::vector<int>::iterator keyframe_id_position = std::find(_keyframe_ids.begin(), _keyframe_ids.end(),
                                                                frame_id);
...
bool Map::TriangulateMaplineByMappoints(MaplinePtr mapline) {
    ...
    endpoints
            << points[min_idx].x, points[min_idx].y, points[min_idx].z, points[max_idx].x, points[max_idx].y, points[max_idx].z;
    mapline->SetEndpoints(endpoints, false);
    mapline->SetEndpointsUpdateStatus(false);
...
void Map::LocalMapOptimization(FramePtr new_frame, MappingOperation &opr) {
    ...
    while (neighbor_frames.size() < frame_num) {   // 仅优化过去的5帧
    ...
    for (size_t i = 0; i < neighbor_frames.size(); ++i) {
        ...
        bool fix_this_frame = ((frame->GetFrameId() == _keyframes.begin()->first) ||
                               (i == neighbor_frames.size() - 1));
        if (fix_this_frame) fixed_frame_num++;

        if (IMUInit()) {
            AddFrameVertex(frame, poses, 0, velocities, biases, imu_constraints, fix_this_frame, !fix_this_frame,
                           fix_this_frame);
        } else {
    ...
    if (fixed_frames.size() > 0 && max_fixed_frame_num > fixed_frame_num) {
        ...
        for (std::set<std::pair<int, FramePtr>>::reverse_iterator rit = ordered_fixed_frames.rbegin();
    ...
    for (auto &mpt: mappoints) {
        ...
        for (auto &kv: obversers) {
            ...
            if (keypoint(2) > 0) {
                ...
                MonoPointConstraintPtr mono_constraint = std::shared_ptr<MonoPointConstraint>(
                        new MonoPointConstraint());
    ...
    for (auto &mpl: maplines) {
        ...
        for (auto &kv: obversers) {
            ...
            if (kf->GetLineRight(kv.second, line_right)) {
                StereoLineConstraintPtr stereo_line_constraint = std::shared_ptr<StereoLineConstraint>(
                        new StereoLineConstraint());
                ...
                MonoLineConstraintPtr mono_line_constraint = std::shared_ptr<MonoLineConstraint>(
                        new MonoLineConstraint());
    ...
    LocalmapOptimization(poses, points, lines, velocities, biases, camera_list, mono_point_constraints,
                         stereo_point_constraints, mono_line_constraints, stereo_line_constraints, imu_constraints,
                         Rwg,
                         _backend_optimization_config);
...
//    opr.reserveKeyFrames(poses.size());
    for (auto &kv: poses) {
        int frame_id = kv.first;
        Pose3d pose = kv.second;

        // 在此处添加keyframe, 固定的帧也需要插入
//        opr.addKeyFrame(_keyframes[frame_id], false);
...
//    opr.reserveMapPoints(points.size());
    for (auto &kv: points) {
        int mpt_id = kv.first;
        ...
        auto point = _mappoints[mpt_id];
//        if (!point->IsRetrived())
//            point->SetRetrived(true);  // 问题：一旦这个点被取走了，就不会给GS了，可是他还会继续优化呀？ 理论上只有被边缘化的时，才给GS，但是GS也会优化这个点的位置
//        opr.addMapPoint(point);
    }
    ...
    // 延迟 frame 给3DGS
    opr.reserveKeyFrames(5);
    int frame_to_gaussian_num = 5;
    auto last_fix_frame = neighbor_frames[0]; // 获得固定帧，本次不参与localBA优化
    std::vector<FramePtr> fixed_frames_to_gaussian;
    fixed_frames_to_gaussian.emplace_back(last_fix_frame);
    opr.addKeyFrame(last_fix_frame, false);

    while (fixed_frames_to_gaussian.size() < frame_to_gaussian_num) {   // 仅优化过去的5帧
        last_fix_frame = last_fix_frame->PreviousFrame();
        if (last_fix_frame)
        {
            std::vector<MappointPtr> &gaussian_mappoints = last_fix_frame->GetAllMappoints();
            for (MappointPtr mpt: gaussian_mappoints) {
                if (!mpt || !mpt->IsValid() || mpt->IsRetrived() || mpt->IsBad())
                    continue;

                if (!mpt->IsRetrived())
                    mpt->SetRetrived(true);
                    opr.addMapPoint(mpt);
            }
            fixed_frames_to_gaussian.push_back(last_fix_frame);
            opr.addKeyFrame(last_fix_frame, false);
        } else
            break; // 说明是第一帧，退出了
    }
...
int Map::UpdateFrameTrackIds(int track_id) {
    for (auto &pair: to_update_track_id) {
        ...
        mpt->SetColor(frame->GetColor(kpt_id));
...
void Map::SaveKeyframeTrajectory(std::string file_path) {
    std::vector<std::pair<std::string , Eigen::Matrix4d>> trajectory_camera, trajectory_imu;
    for (auto &kv: _keyframes) {
        FramePtr kf = kv.second;

        auto camera_pose = kf->GetPose();
        trajectory_camera.emplace_back(std::make_pair(kf->name, camera_pose));

        Eigen::Matrix4d imu_pose = camera_pose * _camera->BodyToCamera();
        trajectory_imu.emplace_back(std::make_pair(kf->name, imu_pose));

    }

    SaveTumTrajectoryToFile(file_path, trajectory_camera);

    std::string imu_path = file_path;
    size_t dot_pos = imu_path.find_last_of('.');
    imu_path.insert(dot_pos, "_imu");
    SaveTumTrajectoryToFile(imu_path, trajectory_imu);


}

void Map::SaveAllframeTrajectory(std::string file_path){
    std::vector<std::pair<std::string, Eigen::Matrix4d>> trajectory_imu, trajectory_camera;
    for (auto &kv: _keyframes) {
        FramePtr kf = kv.second;
        trajectory_camera.emplace_back(std::make_pair(kf->name, kf->GetPose()));
        trajectory_imu.emplace_back(std::make_pair(kf->name, kf->GetPose() * _camera->BodyToCamera()));
    }

    for(auto& normal_data : normal_frame_pose)
    {
        std::string name = std::get<0>(normal_data);
        int RefFrame_id = std::get<1>(normal_data);
        Eigen::Matrix4d T_ref_to_cur = std::get<2>(normal_data);

        if(_keyframes.find(RefFrame_id) == _keyframes.end()) continue;

        Eigen::Matrix4d RefFrame_pose = _keyframes[RefFrame_id]->GetPose();
        Eigen::Matrix4d CurFrame_pose = RefFrame_pose * T_ref_to_cur;
        trajectory_camera.emplace_back(std::make_pair(name, CurFrame_pose));

        Eigen::Matrix4d imu_pose = CurFrame_pose * _camera->BodyToCamera();
        trajectory_imu.emplace_back(std::make_pair(name, imu_pose));


    }

    sortTrajectory(trajectory_imu);
    sortTrajectory(trajectory_camera);
...
void Map::SaveKeyframeTrajectory_Gaussian(std::string file_path, std::map<size_t, Eigen::Matrix4f> kfposes){
    std::vector<std::pair<std::string , Eigen::Matrix4d>> trajectory_camera, trajectory_imu;
    for (auto &kv: kfposes) {
        auto kfid = kv.first;


        Eigen::Matrix4d kfpose = kv.second.cast<double>();
        if(_keyframes.find(kfid) == _keyframes.end()) continue;
        trajectory_camera.emplace_back(std::make_pair(_keyframes[kfid]->name, kfpose));

        Eigen::Matrix4d imu_pose = kfpose * _camera->BodyToCamera();
        trajectory_imu.emplace_back(std::make_pair(_keyframes[kfid]->name, imu_pose));
...
void Map::SaveAllframeTrajectory_Gaussian(std::string file_path, std::map<size_t, Eigen::Matrix4f> kfposes){
    std::vector<std::pair<std::string , Eigen::Matrix4d>> trajectory_camera, trajectory_imu;
    for (auto &kv: kfposes) {
        auto kfid = kv.first;
    ...
    for(auto& normal_data : normal_frame_pose)
    {
        ...
        Eigen::Matrix4d RefFrame_pose = kfposes[RefFrame_id].cast<double>();
        Eigen::Matrix4d CurFrame_pose = RefFrame_pose * T_ref_to_cur;
        trajectory_camera.emplace_back(std::make_pair(name, CurFrame_pose));
    ...
    for (size_t i = 0; i < frame_list.size(); i++) {
        ...
        for (size_t j = 0; j < lines.size() - 1; j++) {
            if (frame_time < times[j + 1] && frame_time >= times[j]) {
                ...
                Eigen::Quaterniond q(std::stod(lines[j][7]), std::stod(lines[j][4]), std::stod(lines[j][5]),
                                     std::stod(lines[j][6]));
                Eigen::Matrix4d pose = Eigen::Matrix4d::Identity();
                pose.block<3, 1>(0, 3) = t;
                std::cout << "frame_time = " << std::setprecision(19) << frame_time << ", line_time = " << times[j]
...
bool Map::InitializeIMU(FramePtr frame) {
    ...
    for (size_t i = 0; i < frame_list.size() - 1; i++) {
        double dist = (frame_list[i]->GetPose().block<3, 1>(0, 3) -
                       frame_list[i + 1]->GetPose().block<3, 1>(0, 3)).norm();
        if (dist < 0.005) {
    ...
    for (size_t i = 0; i < frame_list.size(); i++) {
        ...
        if (i > 0) {
            double dd = (frame->IMUPose().block<3, 1>(0, 3) -
                         frame_list[i - 1]->IMUPose().block<3, 1>(0, 3)).norm();
            std::cout << "frame_id = " << frame->GetFrameId() << ", dt = " << preinteration->dT << ", d = " << dd
                      << ", v = " << (dd / preinteration->dT) << std::endl;
        }
    ...
    for (const MappointPtr mpt: mappoints) {
        ...
        for (const auto &kv: obversers) {
            ...
            if (covi_iter == _covisibile_frames[frame].end()) {
                _covisibile_frames[frame][covi_frame] = 1;     // 这是一个邻接表，表示一个共视图
            } else {
                covi_iter->second++;
            }
        }
    ...
    for (auto kv: _keyframes) {
        ...
        for (size_t i = 0; i < mpts.size(); i++) {
            ...
            if (_mappoints.find(mpt_id_) == _mappoints.end()) {
                std::cout << "frame_id = " << kv.first << ", kpt id = " << i << ", mpt_id_ = " << mpt_id_
                          << ", mpt type = " << mpt->GetType() << std::endl;
            }
        }
        ...
        for (size_t i = 0; i < mpls.size(); i++) {
            ...
            if (_maplines.find(mpl_id_) == _maplines.end()) {
                std::cout << "frame_id = " << kv.first << ", line_2d id = " << i << ", mpl_id_ = " << mpl_id_
                          << ", mpl type = " << mpl->GetType() << std::endl;
            }
...
int Map::GetNumKeyframes(){ return _keyframes.size();}

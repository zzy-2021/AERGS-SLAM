...
void SaveDetectorResult(
        cv::Mat &image, Eigen::Matrix<float, 259, Eigen::Dynamic> &features, std::string save_root,
        std::string idx) {
    cv::Mat img_color;
...
void SaveStereoMatchResult(cv::Mat &image_left, cv::Mat &image_right,
                           Eigen::Matrix<float, 259, Eigen::Dynamic> &features_left,
                           Eigen::Matrix<float, 259, Eigen::Dynamic> &features_right,
                           std::vector<cv::DMatch> &stereo_matches, std::string stereo_save_root, int frame_id) {
    ...
    std::string stereo_save_image_path = ConcatenateFolderAndFileName(stereo_debug_save_dir,
                                                                      stereo_save_image_name);
    SaveMatchingResult(stereo_debug_left, left_keypoints, stereo_debug_right, right_keypoints, stereo_matches,
                       stereo_save_image_path);
}
...
void SaveTrackingResult(cv::Mat &last_image, cv::Mat &image, FramePtr last_frame, FramePtr frame,
    ...
    std::string save_image_name =
            "matching_" + std::to_string(last_frame_id) + "_" + std::to_string(frame_id) + ".jpg";
...
void SaveLineDetectionResult(cv::Mat &image, std::vector<Eigen::Vector4d> &lines, std::string save_root,
                             std::string idx) {
    cv::Mat img_color;
    cv::cvtColor(image, img_color, cv::COLOR_GRAY2RGB);
...
cv::Mat DrawLinePointRelation(cv::Mat &image, const Eigen::Matrix<float, 259, Eigen::Dynamic> &features,
                              const std::vector<Eigen::Vector4d> &lines,
                              const std::vector<std::map<int, double>> &points_on_line,
...
void SaveStereoLineMatch(cv::Mat &image_left, cv::Mat &image_right,
    ...
    cv::Mat img_left_color = DrawLinePointRelation(image_left, feature_left, lines_left, points_on_line_left,
                                                   line_ids_left);
    cv::Mat img_right_color = DrawLinePointRelation(image_right, feature_right, lines_right, points_on_line_right,
                                                    line_ids_right);
...
cv::Mat DrawLineWithText(cv::Mat &image, std::vector<Eigen::Vector4d> &lines, std::vector<int> &track_ids) {
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        cv::putText(img_color, std::to_string(track_ids[i]),
                    cv::Point((int) ((line(0) + line(2)) / 2), (int) ((line(1) + line(3)) / 2)),
...
void DrawDbowMatchingResults(FramePtr query_frame, cv::Mat &query_image, std::vector<FramePtr> &database_frames,
    ...
    cv::Mat query_drawed_image = DrawLinePointRelation(query_image, query_features, query_lines,
                                                       query_points_on_line, query_line_ids);
    ...
    for (FramePtr database_frame: database_frames) {
        ...
        cv::Mat base_drawed_image = DrawLinePointRelation(base_image, base_features, base_lines,
                                                          base_points_on_line, base_line_ids);
        ...
        cv::Mat save_image;
        cv::drawMatches(query_drawed_image, query_keypoints, base_drawed_image, base_keypoints, matches,
                        save_image);
...
void DrawDbowJunctionMatchingResults(FramePtr query_frame, cv::Mat &query_image, FramePtr database_frame,
                                     std::vector<std::vector<bool>> &match_matrix, std::string image_root,
    ...
    std::function<cv::Mat(cv::Mat &, std::vector<Eigen::Vector4d> &)> draw_lines = [&](cv::Mat &image,

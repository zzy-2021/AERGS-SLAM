...
RosPublisher::RosPublisher(const RosPublisherConfig &ros_publisher_config, ros::NodeHandle nh) : _config(
        ros_publisher_config) {
        ...
        std::function<void(const FeatureMessgaeConstPtr &)> publish_feature_function =
                [&](const FeatureMessgaeConstPtr &feature_message) {
                    ...
                    if (feature_message->fm_type == FeatureMessgaeType::RelocFeature) {
                        result = DrawFeatures(feature_message->image, feature_message->keypoints,
                        ...
                        result = DrawMatches(feature_message->key_image, drawed_image,
                                             feature_message->keyframe_keypoints, feature_message->keypoints,
                                             feature_message->matches);

                        cv::copyMakeBorder(result, result, 50, 0, 0, 0, cv::BORDER_CONSTANT,
                                           cv::Scalar(0, 0, 0, 255));
                        cv::putText(result, "Feature Detection", cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1,
                                    cv::Scalar(0, 0, 255), 2);
                        std::string keyframe_info =
                                "Keyframe Num : " + std::to_string(feature_message->keyfrmae_id);
                        cv::putText(result, keyframe_info, cv::Point(10 + drawed_image.cols, 30),
                                    cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 2);
                        std::string frame_info = "Current Frame Id: " + std::to_string(feature_message->frame_id);
                        cv::putText(result, frame_info, cv::Point(10 + drawed_image.cols * 2, 30),
                                    cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 2);
                    }

                    sensor_msgs::ImagePtr ros_feature_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8",
                                                                               result).toImageMsg();

...
bool MapUser::filter_matches(
        const std::vector<cv::DMatch>& ref_match,
        const std::vector<cv::DMatch>& matches_render2ref,
        const std::vector<cv::DMatch>& matches_render2query,
        std::vector<cv::DMatch>& match_E  // 通过引用返回结果
) {
    // 第一步：收集ref_match中的所有trainIdx到哈希集合中
    std::unordered_set<int> ref_train_indices;
    ref_train_indices.reserve(ref_match.size());
    for (const auto& match : ref_match) {
        ref_train_indices.insert(match.trainIdx);
    }

    // 第二步：筛选得到match_D (trainIdx不在ref_match的trainIdx中)
    std::vector<cv::DMatch> match_D;
    match_D.reserve(matches_render2ref.size());
    for (const auto& match : matches_render2ref) {
        if (ref_train_indices.find(match.trainIdx) == ref_train_indices.end()) {
            match_D.push_back(match);
        }
    }

    // 如果找不到match_D，直接返回false
    if (match_D.empty()) {
        return false;
    }

    // 第三步：收集matches_render2query中的所有queryIdx到哈希集合中
    std::unordered_set<int> render_query_indices;
    render_query_indices.reserve(matches_render2query.size());
    for (const auto& match : matches_render2query) {
        render_query_indices.insert(match.queryIdx);
    }

    // 第四步：筛选得到match_E (queryIdx存在于matches_render2query的queryIdx中)
    match_E.reserve(match_D.size());
    for (const auto& match : match_D) {
        if (render_query_indices.find(match.queryIdx) != render_query_indices.end()) {
            match_E.push_back(match);
        }
    }

    // 如果找不到match_E，返回false；否则返回true
    return !match_E.empty();
}



bool MapUser::Relocalization(cv::Mat &image, Eigen::Matrix4d &pose, std::vector<FramePtr>& ref_frames, std::vector<int>& ref_frameIds) {  // 把relocalization分为几个步骤
    cv::Mat image_rect;
    ...
    FramePtr frame = std::shared_ptr<Frame>(new Frame(new_frame_id++, false, _camera, 0, image_rect, std::to_string(new_frame_id++)));
    ...
    _junction_database->FrameToBow(junctions, junction_word_features, junction_bow_vector,
                                   junction_word_of_features);
    ...
    if (print_debug_info) {
        ...
        for (auto &kv: frame_scores) {
            ...
            std::cout << "frmae time = " << std::fixed << std::setprecision(9) << frame_time << ", score = "
                      << kv.second << std::endl;
        }
    }
    ...
    for (; fs_it != frame_scores.end(); fs_it++) {
        ...
        if (relocalization_group_iter == group_candidates.end() ||
            relocalization_group_iter->second.group_score < group_candidate.group_score) {
    ...
    if (print_debug_info) {
        ...
        for (auto &kv: group_candidates) {
            double frame_time = kv.first->GetTimestamp();
            std::cout << "frmae time = " << std::fixed << std::setprecision(9) << frame_time << ", group_score = "
                      << kv.second.group_score << std::endl;
        }
    }
    ...
    // sorting of group
    std::vector<std::pair<FramePtr, RelocalizationGroupCandidate>> group_vector(group_candidates.begin(),
    ...
    if (print_debug_info) {
        ...
        for (auto &kv: group_vector) {
            double frame_time = kv.first->GetTimestamp();
            double junction_frame_scores = _junction_database->Score(
                    _junction_database->_frame_bow_vectors[kv.first], junction_bow_vector);
    ...
    for (auto &kv: group_vector) {
        ...
        double junction_frame_scores = _junction_database->Score(_junction_database->_frame_bow_vectors[kv.first],
                                                                 junction_bow_vector);
        ...
        if (print_debug_info) {
            std::cout << "frmae time = " << std::fixed << std::setprecision(9) << kf->GetTimestamp()
                      << ", rate = " << rate
                      << ", score = " << _junction_database->Score(_junction_database->_frame_bow_vectors[kv.first],
                                                                   junction_bow_vector)
                      << ", match_num = " << match_num
    ...
    if (print_debug_info) {
        std::cout << "======================= re-sorting of group after detecting sentences =================  "
                  << std::endl;
        for (auto &kv: group_vector) {
            double frame_time = kv.first->GetTimestamp();
            std::cout << "frmae time = " << std::fixed << std::setprecision(9) << frame_time << ", group_score = "
    ...
    // feature matching   // 1. 在这里，接受一些3DGS的rendering image, 增强视觉定位的精度
    ...
    for (size_t i = 0; i < GoodCandidateNum; i++) {
        FramePtr good_candidate = group_vector[i].first;       // 1. 把good_candidata返回， 把frame返回


        ref_frames.emplace_back(good_candidate);
        ref_frameIds.emplace_back(good_candidate->GetFrameId());

        std::cout<<"get a candidate frame! " << std::endl;
...
CameraPtr MapUser::GetCamera(){
    return this->_camera;
}

std::map<int, FramePtr> MapUser::Getkeyframes()
{
    return this->_map->GetAllKeyframes();
}

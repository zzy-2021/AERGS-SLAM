...
MapRefiner::MapRefiner(MapRefinementConfigs &configs, ros::NodeHandle nh) : odometry_length(0),
                                                                            _configs(configs), _stop(false),
                                                                            _stopped(false), _map_ready(false) {
...
MapRefiner::MapRefiner(MapRefinementConfigs &configs, ros::NodeHandle nh, MapPtr map, std::string &voc_path) {
...
void
MapRefiner::LoopDetection(FramePtr frame, DBoW2::WordIdToFeatures &word_features, DBoW2::BowVector &bow_vector) {
    ...
    for (; fs_it != frame_scores.end(); fs_it++) {
        ...
        if (loop_group_iter == group_candidates.end() ||
            loop_group_iter->second.group_score < group_candidate.group_score) {
    ...
    // sorting of group
    std::vector<std::pair<FramePtr, LoopGroupCandidate>> group_vector(group_candidates.begin(),
...
void MapRefiner::RelativatePoseEstimation(FramePtr frame, DBoW2::WordIdToFeatures &word_features,
                                          FramePtr loop_frame, std::vector<cv::DMatch> &loop_matches,
                                          std::map<FramePtr, LoopGroupCandidate> &group_candidates) {
    int frame_id = frame->GetFrameId();
    ...
    std::function<bool(const Eigen::Vector2d &, const Eigen::Vector2d &)> check_epipolar = [&](
    ...
    std::function<bool(const int &, const MappointPtr)> check_reprojection_error = [&](const int &idx,
                                                                                       const MappointPtr mpt) {
    ...
    std::function<MappointPtr(const int &, const DBoW2::WordId &)> find_more_matches_in_group = [&](const int &idx,
...
void MapRefiner::PoseGraphRefinement() {
    ...
    MappingOperation opr(MappingOperation::OprType::MapRefinement);
    ...
    this->_map->pushMappingOperation(opr);
...
void MapRefiner::MergeMap() {
    ...
    MappingOperation opr(MappingOperation::OprType::MapRefinement);
    GlobalBA(_map, _configs.map_optimization_config, true, true, 10, 10, opr);
    this->_map->pushMappingOperation(opr);
    MergeMaplines();
    ...
    for (auto &kv: _map->_keyframes) {
        ...
        for (int i = 0; i < points_on_lines.size(); i++) {
            ...
            for (const auto &points_and_distances: points_on_lines[i]) {
                ...
                if (mpt && mpl && _map->_mappoints.count(mpt->GetId()) > 0 &&
                    _map->_maplines.count(mpl->GetId()) > 0) {
...
void MapRefiner::BuildJunctionDatabase() {
    ...
    SuperpointVocabularyPtr jun_voc = std::shared_ptr<SuperpointVocabulary>(
            new SuperpointVocabulary(k, L, weight, scoring));
...
void MapRefiner::SaveTrajectory(std::string save_path) { // keyframe
    _map_mutex.lock();
    _map->SaveKeyframeTrajectory(save_path);
    _map_mutex.unlock();
}

void MapRefiner::SaveAllFrameTrajectory(std::string save_path) { // allkeyframe
    _map_mutex.lock();
    _map->SaveAllframeTrajectory(save_path);
    _map_mutex.unlock();
}
...
void MapRefiner::GlobalMapOptimization() {
    _map_mutex.lock();
    MappingOperation opr(MappingOperation::OprType::MapRefinement);
    GlobalBA(_map, _configs.map_optimization_config, true, true, 50, 40, opr);
    this->_map->pushMappingOperation(opr);
    _map_mutex.unlock();
}
...
void MapRefiner::StartRefining(std::string& map_root) // 在vo结束后执行
{
    // 一些初始设置
    _map_mutex.lock();
    _map->SetIMUInit(false);
    ...
    GlobalMapOptimization();  // 3
    UpdateCovisibilityGraph();
    std::cout << "Done." << std::endl;

    std::cout << "Build junction database..." << std::endl;
    BuildJunctionDatabase();
    std::cout << "Done." << std::endl;

//    std::cout << "Updata normal frame pose..." << std::endl;
//    std::string trajectory_keyframe_path = ConcatenateFolderAndFileName(map_root, "KeyFrameTrajectory_TUM.txt");
//    SaveTrajectory(trajectory_keyframe_path);
//
//
//    std::string trajectory_allframe_path = ConcatenateFolderAndFileName(map_root, "CameraTrajectory_TUM.txt");
//    SaveAllFrameTrajectory(trajectory_allframe_path);

    std::cout << "Saving final map..." << std::endl;
    SaveFinalMap(map_root);
    std::cout << "Done." << std::endl;

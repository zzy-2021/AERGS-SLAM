...
SuperPointLightGlue::SuperPointLightGlue(const PointMatcherConfig &lightglue_config) : lightglue_config_(
        lightglue_config), engine_(nullptr) {
...
bool SuperPointLightGlue::build() {
    ...
    profile->setDimensions(lightglue_config_.input_tensor_names[0].c_str(), nvinfer1::OptProfileSelector::kMIN,
                           nvinfer1::Dims3(1, 1, 2));
    profile->setDimensions(lightglue_config_.input_tensor_names[0].c_str(), nvinfer1::OptProfileSelector::kOPT,
                           nvinfer1::Dims3(1, 512, 2));
    profile->setDimensions(lightglue_config_.input_tensor_names[0].c_str(), nvinfer1::OptProfileSelector::kMAX,
                           nvinfer1::Dims3(1, 1024, 2));

    profile->setDimensions(lightglue_config_.input_tensor_names[1].c_str(), nvinfer1::OptProfileSelector::kMIN,
                           nvinfer1::Dims3(1, 1, 2));
    profile->setDimensions(lightglue_config_.input_tensor_names[1].c_str(), nvinfer1::OptProfileSelector::kOPT,
                           nvinfer1::Dims3(1, 512, 2));
    profile->setDimensions(lightglue_config_.input_tensor_names[1].c_str(), nvinfer1::OptProfileSelector::kMAX,
                           nvinfer1::Dims3(1, 1024, 2));

    profile->setDimensions(lightglue_config_.input_tensor_names[2].c_str(), nvinfer1::OptProfileSelector::kMIN,
                           nvinfer1::Dims3(1, 1, 256));
    profile->setDimensions(lightglue_config_.input_tensor_names[2].c_str(), nvinfer1::OptProfileSelector::kOPT,
                           nvinfer1::Dims3(1, 512, 256));
    profile->setDimensions(lightglue_config_.input_tensor_names[2].c_str(), nvinfer1::OptProfileSelector::kMAX,
                           nvinfer1::Dims3(1, 1024, 256));

    profile->setDimensions(lightglue_config_.input_tensor_names[3].c_str(), nvinfer1::OptProfileSelector::kMIN,
                           nvinfer1::Dims3(1, 1, 256));
    profile->setDimensions(lightglue_config_.input_tensor_names[3].c_str(), nvinfer1::OptProfileSelector::kOPT,
                           nvinfer1::Dims3(1, 512, 256));
    profile->setDimensions(lightglue_config_.input_tensor_names[3].c_str(), nvinfer1::OptProfileSelector::kMAX,
                           nvinfer1::Dims3(1, 1024, 256));
...
bool SuperPointLightGlue::construct_network(TensorRTUniquePtr<nvinfer1::IBuilder> &builder,
...
bool SuperPointLightGlue::infer(const Eigen::Matrix<float, 258, Eigen::Dynamic> &features0,
...
bool SuperPointLightGlue::process_input(const BufferManager &buffers,
    ...
    auto *descriptors_0_buffer = static_cast<float *>(buffers.getHostBuffer(
            lightglue_config_.input_tensor_names[2]));
    auto *descriptors_1_buffer = static_cast<float *>(buffers.getHostBuffer(
            lightglue_config_.input_tensor_names[3]));
...
bool SuperPointLightGlue::process_output(const BufferManager &buffers,
                                         Eigen::Matrix<int, Eigen::Dynamic, 2> &matches_index,
                                         Eigen::Matrix<float, Eigen::Dynamic, 1> &matches_score) {

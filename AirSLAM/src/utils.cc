...
double StringTimeToDouble(std::string time_str) {
    time_str.erase(std::remove_if(time_str.begin(), time_str.end(), [](char c) { return c == '.'; }),
                   time_str.end());
...
cv::Mat DrawFeatures(const cv::Mat &image, const std::vector<cv::KeyPoint> &keypoints,
                     ...
                     const std::vector<int> &line_track_ids,
                     const std::vector<std::map<int, double>> &points_on_lines) {
    cv::Mat img_color;
    cv::cvtColor(image, img_color, cv::COLOR_GRAY2RGB);
...
cv::Mat DrawMatches(const cv::Mat &ref_image, const cv::Mat &image, const std::vector<cv::KeyPoint> &ref_kpts,
    ...
    for (size_t i = 0; i < matches.size(); i++) {
        cv::line(rgba_image,
                 cv::Point2f(ref_kpts[matches[i].queryIdx].pt.x + image.cols, ref_kpts[matches[i].queryIdx].pt.y),
...
void sortTrajectory(std::vector<std::pair<std::string, Eigen::Matrix4d>>& trajectory) {
    // 使用std::sort和自定义比较函数
    std::sort(trajectory.begin(), trajectory.end(),
              [](const std::pair<std::string, Eigen::Matrix4d>& a,
                 const std::pair<std::string, Eigen::Matrix4d>& b) {

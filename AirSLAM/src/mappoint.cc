...
Mappoint::Mappoint(int &mappoint_id) : tracking_frame_id(-1), last_frame_seen(-1),
                                       local_map_optimization_frame_id(-1), _id(mappoint_id),
                                       _type(Type::UnTriangulated) {
...
void Mappoint::SetRetrived(const bool bretrived) {
    retrived = bretrived;
}
...
void Mappoint::SetColor(const Eigen::Vector3d &c) {
    _color = c;
}

Eigen::Vector3d Mappoint::GetColor() {
    return _color;
}

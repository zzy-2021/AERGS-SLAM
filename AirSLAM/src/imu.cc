...
void ComputerDeltaR(const Eigen::Vector3d &rv, Eigen::Matrix3d &delta_R, Eigen::Matrix3d &Jr) {
    ...
    if (d < IMU_EPS) {
        ...
        Jr = Eigen::Matrix3d::Identity() - ((1.0 - cos(d)) / d2) * rv_hat +
             ((d - sin(d)) / (d2 * d)) * rv_hat * rv_hat;
    }
}

Eigen::Vector3d
VectorInterpolation(const Eigen::Vector3d &v0, const Eigen::Vector3d &v1, double t0, double t1, double t) {
...
void Preinteration::AddBatchData(const ImuDataList &batch_imu_data, double t0, double t1) {
    ...
    for (; i < batch_imu_data.size() - 1; i++) {
        ...
        mid_gyr = VectorInterpolation(batch_imu_data[i].gyr, batch_imu_data[i + 1].gyr, batch_imu_data[i].timestamp,
                                      batch_imu_data[i + 1].timestamp, mid_t);
        mid_acc = VectorInterpolation(batch_imu_data[i].acc, batch_imu_data[i + 1].acc, batch_imu_data[i].timestamp,
                                      batch_imu_data[i + 1].timestamp, mid_t);
...
Preinteration::GetDeltaPosition(const Eigen::Vector3d &gyr_bias, const Eigen::Vector3d &acc_bias) {
    return dP + JPg * (gyr_bias - bg) + JPa * (acc_bias - ba);
}

const Eigen::Vector3d
Preinteration::GetDeltaVelocity(const Eigen::Vector3d &gyr_bias, const Eigen::Vector3d &acc_bias) {
...
void Preinteration::Predict(const Eigen::Matrix4d &Twb0, const Eigen::Vector3d &vwb0, Eigen::Matrix4d &Twb1,
                            Eigen::Vector3d &vwb1) {
    if (Valid()) {

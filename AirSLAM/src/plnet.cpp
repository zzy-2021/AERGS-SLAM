...
bool PLNet::build() {
    ...
    auto builder_stage1 = TensorRTUniquePtr<nvinfer1::IBuilder>(
    ...
    auto network_stage1 = TensorRTUniquePtr<nvinfer1::INetworkDefinition>(
            builder_stage1->createNetworkV2(explicit_batch));
    if (!network_stage1) {
        return false;
    }
    ...
    auto parser_stage1 = TensorRTUniquePtr<nvonnxparser::IParser>(
            nvonnxparser::createParser(*network_stage1, gLogger.getTRTLogger()));
    if (!parser_stage1) {
        return false;
    }
    ...
    auto constructed_stage1 = construct_network_stage1(builder_stage1, network_stage1, config_stage1,
                                                       parser_stage1);
    ...
    TensorRTUniquePtr<nvinfer1::IHostMemory> plan_stage1{
            builder_stage1->buildSerializedNetwork(*network_stage1, *config_stage1)};
    if (!plan_stage1) {
        return false;
    }
    ...
    engine0_ = std::shared_ptr<nvinfer1::ICudaEngine>(
            runtime_stage1->deserializeCudaEngine(plan_stage1->data(), plan_stage1->size()));
    if (!engine0_) {
        return false;
    }
    ...
    auto builder_stage2 = TensorRTUniquePtr<nvinfer1::IBuilder>(
            nvinfer1::createInferBuilder(gLogger.getTRTLogger()));
    if (!builder_stage2) {
        return false;
    }
    auto network_stage2 = TensorRTUniquePtr<nvinfer1::INetworkDefinition>(
            builder_stage2->createNetworkV2(explicit_batch));
    if (!network_stage2) {
        return false;
    }
    ...
    auto parser_stage2 = TensorRTUniquePtr<nvonnxparser::IParser>(
            nvonnxparser::createParser(*network_stage2, gLogger.getTRTLogger()));
    if (!parser_stage2) {
        return false;
    }
    ...
    profile_stage2->setDimensions("idx_lines_for_junctions", nvinfer1::OptProfileSelector::kMIN,
                                  nvinfer1::Dims2(1, 2));
    profile_stage2->setDimensions("idx_lines_for_junctions", nvinfer1::OptProfileSelector::kOPT,
                                  nvinfer1::Dims2(20000, 2));
    profile_stage2->setDimensions("idx_lines_for_junctions", nvinfer1::OptProfileSelector::kMAX,
                                  nvinfer1::Dims2(50000, 2));
    ...
    profile_stage2->setDimensions("loi_features", nvinfer1::OptProfileSelector::kMIN,
                                  nvinfer1::Dims4(1, 16, 16, 16));
    profile_stage2->setDimensions("loi_features", nvinfer1::OptProfileSelector::kOPT,
                                  nvinfer1::Dims4(1, 128, 128, 128));
    profile_stage2->setDimensions("loi_features", nvinfer1::OptProfileSelector::kMAX,
                                  nvinfer1::Dims4(1, 512, 512, 512));
    profile_stage2->setDimensions("loi_features_thin", nvinfer1::OptProfileSelector::kMIN,
                                  nvinfer1::Dims4(1, 4, 16, 16));
    profile_stage2->setDimensions("loi_features_thin", nvinfer1::OptProfileSelector::kOPT,
                                  nvinfer1::Dims4(1, 4, 128, 128));
    profile_stage2->setDimensions("loi_features_thin", nvinfer1::OptProfileSelector::kMAX,
                                  nvinfer1::Dims4(1, 4, 512, 512));
    profile_stage2->setDimensions("loi_features_aux", nvinfer1::OptProfileSelector::kMIN,
                                  nvinfer1::Dims4(1, 4, 16, 16));
    profile_stage2->setDimensions("loi_features_aux", nvinfer1::OptProfileSelector::kOPT,
                                  nvinfer1::Dims4(1, 4, 128, 128));
    profile_stage2->setDimensions("loi_features_aux", nvinfer1::OptProfileSelector::kMAX,
                                  nvinfer1::Dims4(1, 4, 512, 512));
    config_stage2->addOptimizationProfile(profile_stage2);

    auto constructed_stage2 = construct_network_stage2(builder_stage2, network_stage2, config_stage2,
                                                       parser_stage2);
    ...
    TensorRTUniquePtr<nvinfer1::IHostMemory> plan_stage2{
            builder_stage2->buildSerializedNetwork(*network_stage2, *config_stage2)};
    if (!plan_stage2) {
        return false;
    }
    ...
    engine1_ = std::shared_ptr<nvinfer1::ICudaEngine>(
            runtime_stage2->deserializeCudaEngine(plan_stage2->data(), plan_stage2->size()));
    if (!engine1_) {
        return false;
    }
    save_engine();
...
bool PLNet::construct_network_stage1(TensorRTUniquePtr<nvinfer1::IBuilder> &builder,
                                     TensorRTUniquePtr<nvinfer1::INetworkDefinition> &network,
                                     TensorRTUniquePtr<nvinfer1::IBuilderConfig> &config,
                                     TensorRTUniquePtr<nvonnxparser::IParser> &parser) const {
    auto parsed = parser->parseFromFile(plnet_config_.plnet_s0_onnx.c_str(),
...
bool PLNet::construct_network_stage2(TensorRTUniquePtr<nvinfer1::IBuilder> &builder,
                                     TensorRTUniquePtr<nvinfer1::INetworkDefinition> &network,
                                     TensorRTUniquePtr<nvinfer1::IBuilderConfig> &config,
                                     TensorRTUniquePtr<nvonnxparser::IParser> &parser) const {
    auto parsed = parser->parseFromFile(plnet_config_.plnet_s1_onnx.c_str(),
                                        static_cast<int>(gLogger.getReportableSeverity()));
    if (!parsed) {
        return false;
    }
...
bool
PLNet::wireframe_matcher(const float *iskeep, const float *idx_junc_to_end_min, const float *idx_junc_to_end_max) {
...
void
PLNet::extract_descriptors(const float *descriptors, Eigen::Matrix<float, 259, Eigen::Dynamic> &features, int h,
                           int w, int s) {
    ...
    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> descriptor_matrix = features.block(3, 0,
                                                                                            features.rows() - 3,
                                                                                            features.cols());
...
bool PLNet::keypoints_decoder(const float *scores, const float *descriptors,
                              Eigen::Matrix<float, 259, Eigen::Dynamic> &features) {
    detect_point(scores, features, resized_height, resized_width, plnet_config_.keypoint_threshold,
                 plnet_config_.remove_borders, plnet_config_.max_keypoints);
...
bool PLNet::junction_detector(const float *scores, const float *descriptors,
                              std::vector<std::vector<bool>> &junction_map,
...
bool PLNet::process_output(const BufferManager &buffers, Eigen::Matrix<float, 259, Eigen::Dynamic> &features,
                           ...
                           Eigen::Matrix<float, 259, Eigen::Dynamic> &junctions, bool junction_detection) {
    ...
    context1_->setBindingDimensions(idx_lines_for_junctions_index_,
                                    nvinfer1::Dims2((int) idx_lines_for_junctions_unique_.size(), 2));
    ...
    for (int i = 0; i < idx_lines_for_junctions_unique_.size(); ++i) {
        ...
        int yi2 = (int) (y2 + 0.1);
        int border = std::max(plnet_config_.remove_borders, 0);
        bool p1_valid = (xi1 > border) && (xi1 < resized_width - border) && (yi1 > border) &&
                        (yi1 < resized_height - border);
        bool p2_valid = (xi2 > border) && (xi2 < resized_width - border) && (yi2 > border) &&
                        (yi2 < resized_height - border);

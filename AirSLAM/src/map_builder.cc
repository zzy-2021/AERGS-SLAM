...
MapBuilder::MapBuilder(VisualOdometryConfigs &configs, MapRefinementConfigs &mr_configs, std::string& voc_path, ros::NodeHandle nh) : _shutdown(false),
                                                                             _feature_thread_stop(false),
                                                                             _tracking_trhead_stop(false),
                                                                             _init(false),
                                                                             _insert_next_keyframe(false),
                                                                             _track_id(0),
                                                                             _line_track_id(0), _configs(configs) {
    _camera = std::shared_ptr<Camera>(new Camera(configs.camera_config_path));
    _preinteration_keyframe.SetNoiseAndWalk(_camera->GyrNoise(), _camera->AccNoise(), _camera->GyrWalk(),
                                            _camera->AccWalk());
    ...
    // 定义一个 map_refiner,用于在VO之后的地图优化
    _map_refiner = std::shared_ptr<MapRefiner>(new MapRefiner(mr_configs, nh, _map,  voc_path));
...
void MapBuilder::ExtractFeatureThread() {
    while (!_shutdown || !_data_buffer.empty()) {
        ...
        cv::Mat image_left_rect = input_data->image_left.clone();    // RGB or Gray
        cv::Mat image_right_rect = input_data->image_right.clone();
        cv::Mat image_to_Gaussian = input_data->image_left.clone();  // RGB or Gray
        std::string name = input_data->name;

        if (_camera->IsBGR() && image_to_Gaussian.channels() == 3)
            cv::cvtColor(image_to_Gaussian, image_to_Gaussian, cv::COLOR_BGR2RGB);  // 3DGS 必须接受 RGB 格式的图像

        if (image_left_rect.channels() == 3)
        {
            cv::cvtColor(image_left_rect, image_left_rect, cv::COLOR_BGR2GRAY);
            cv::cvtColor(image_right_rect, image_right_rect, cv::COLOR_BGR2GRAY);

            input_data->image_left = image_left_rect.clone();
        }
        if (image_left_rect.channels() == 4)
        {
            cv::cvtColor(image_left_rect, image_left_rect, cv::COLOR_BGRA2GRAY);
            cv::cvtColor(image_right_rect, image_right_rect, cv::COLOR_BGRA2GRAY);

            input_data->image_left = image_left_rect.clone();
        }

        // construct frame
        FramePtr frame = std::shared_ptr<Frame>(new Frame(frame_id, false, _camera, timestamp, image_to_Gaussian, name));
        ...
        if (!_init || _insert_next_keyframe) {
            ...
            _feature_detector->Detect(image_left_rect, image_right_rect, left_features, right_features, left_lines,
                                      right_lines, junctions);
...
void MapBuilder::TrackingThread() {
    while (!_shutdown || !_data_buffer.empty() || !_tracking_data_buffer.empty()) {
        ...
        if (frame_type == FrameType::NormalFrame){
            ...
            int ref_frame_id = ref_keyframe->GetFrameId();
            Eigen::Matrix4d ref_pose = ref_keyframe->GetPose();
            Eigen::Matrix4d cur_pose = frame->GetPose();
            Eigen::Matrix4d T_ref_to_cur = ref_pose.inverse() * cur_pose;

            _map->normal_frame_pose.emplace_back(std::make_tuple(name, ref_frame_id, T_ref_to_cur));
        }
...
//            std::cout << "insert keyframe, id = " << frame->GetFrameId() << std::endl;
        ...
        PublishFrame(frame, image_left_rect, frame_type, matches);   // publish时，最好能发布3DGS的 render_image
    }
...
int MapBuilder::TrackFrame(FramePtr ref_frame, FramePtr current_frame, std::vector<cv::DMatch> &matches,
                           Preinteration &_preinteration) {
    // line tracking
    ...
    MatchLines(ref_points_on_lines, current_points_on_lines, matches, ref_features.cols(), current_features.cols(),
               line_matches);
...
int MapBuilder::FramePoseOptimization(FramePtr frame0, FramePtr frame1, std::vector<MappointPtr> &mappoints,
    ...
    int num_inliers = FrameOptimization(poses, points, lines, velocities, biases, camera_list,
                                        mono_point_constraints, stereo_point_constraints, mono_line_constraints,
                                        stereo_line_constraints,
...
int
MapBuilder::AddKeyframeCheck(FramePtr ref_keyframe, FramePtr current_frame,
    ...
    if ((float) match_num / ref_features.cols() < feature_tracking_thr ||
        (float) match_num / current_features.cols() < feature_tracking_thr ||
        match_num < _configs.keyframe_config.max_num_match) {
        return 1;
    }
...
void MapBuilder::InsertKeyframe(FramePtr frame) {
    ...
    {
        std::unique_lock<std::mutex> lock_map(_map->mMutexMapUpdate);
...
void
MapBuilder::PublishFrame(FramePtr frame, cv::Mat &image, FrameType frame_type, std::vector<cv::DMatch> &matches) {
...
void MapBuilder::SaveAllFrameTrajectory(std::string file_path) {
    _map->SaveAllframeTrajectory(file_path);
}


void MapBuilder::SaveAllFrameTrajectory_Gaussian(std::string file_path, std::map<size_t, Eigen::Matrix4f> kfposes){
    _map->SaveAllframeTrajectory_Gaussian(file_path, kfposes);
}

void MapBuilder::SaveTrajectory_Gaussian(std::string file_path, std::map<size_t, Eigen::Matrix4f> kfposes){
    _map->SaveKeyframeTrajectory_Gaussian(file_path, kfposes);
}
...
CameraPtr MapBuilder::GetCameraPtr() {
    return _camera;
}

MapPtr MapBuilder::GetMapPtr() {
    return _map;
}

void MapBuilder::Refining(){
    _map_refiner->StartRefining(_configs.saving_dir);
    _refined = true;

    return;
}

bool MapBuilder::IsRefined()
{
    return _refined;
}

...
Dataset::Dataset(const std::string &dataroot, const bool use_imu) : _use_imu(use_imu) {
    ...
    for (size_t i = 0; i < image_names.size(); ++i) {
        ...
        _image_names.emplace_back(image_names[i].substr(0, image_names[i].find_last_of('.')));
    }
...
bool Dataset::GetData(size_t idx, cv::Mat &left_image, cv::Mat &right_image, ImuDataList &batch_imu_data,
                      double &timestamp, std::string& name) {
    ...
    left_image = cv::imread(_left_images[idx], cv::IMREAD_UNCHANGED);
    right_image = cv::imread(_right_images[idx], 0);
    timestamp = _timestamps[idx];
    name = _image_names[idx];

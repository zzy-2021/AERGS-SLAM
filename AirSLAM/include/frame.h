...
public:
    ...
    Frame(int frame_id, bool pose_fixed, CameraPtr camera, double timestamp, cv::Mat imgleft, std::string name);
    ...
    void
    AddLeftFeatures(Eigen::Matrix<float, 259, Eigen::Dynamic> &features_left,
                    std::vector<Eigen::Vector4d> &lines_left);

    int AddRightFeatures(Eigen::Matrix<float, 259, Eigen::Dynamic> &features_right,
                         std::vector<Eigen::Vector4d> &lines_right, std::vector<cv::DMatch> &stereo_matches);
    ...
    Eigen::Vector3d GetColor(int idx); // 获得cv::Mat图像中，idx的特征对应的颜色。

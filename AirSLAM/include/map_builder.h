...
class MapBuilder {
public:
    MapBuilder(VisualOdometryConfigs &configs, MapRefinementConfigs &mr_configs, std::string& voc_path, ros::NodeHandle nh);
    ...
    int TrackFrame(FramePtr ref_frame, FramePtr current_frame, std::vector<cv::DMatch> &matches,
                   Preinteration &_preinteration);

    int FramePoseOptimization(FramePtr frame0, FramePtr frame, std::vector<MappointPtr> &mappoints,
    ...
    void SaveAllFrameTrajectory(std::string file_path);

    void SaveAllFrameTrajectory_Gaussian(std::string file_path, std::map<size_t, Eigen::Matrix4f> kfposes);

    void SaveTrajectory_Gaussian(std::string file_path, std::map<size_t, Eigen::Matrix4f> kfposes);

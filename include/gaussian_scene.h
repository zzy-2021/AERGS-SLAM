...
public:
    void addCamera(GaussianView& camera);
    GaussianView& getCamera(camera_id_t cameraId);

    void remove_camera_outside_window();
...
public:
    ...
    std::map<camera_id_t, GaussianView> cameras_;
    std::map<std::size_t, std::shared_ptr<GaussianKeyframe>> keyframes_;
    std::map<std::size_t, std::shared_ptr<GaussianKeyframe>> keyframes_saved;
